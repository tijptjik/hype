...
import { planLayerPreviewRefreshJobs } from '$lib/map/previews/render.server'
...
describe('layer preview refresh planning', () => {
  beforeEach(() => {
  ...
  it('plans force-refresh jobs for unpublished layers', async () => {
    ...
    const jobs = await planLayerPreviewRefreshJobs(
      db as never,
    ...
    expect(jobs[0]).toMatchObject({
      ...
      sourceUrl:
        'https://preview.hype.hk/headless/map-layer-preview/layer-draft?token=secret-token',
    })
  })
  ...
  it('plans incremental refresh jobs when an unpublished layer changes', async () => {
    ...
    expect(jobs[0]).toMatchObject({
      kind: 'layers',
      identifier: 'layer-draft',
      sourceUrl: 'https://preview.hype.hk/headless/map-layer-preview/layer-draft',
    })
  })
})
