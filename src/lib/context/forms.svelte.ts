...
class OrganisationForm {
  ...
  constructor(data, isNew) {
    ...
    const formConfig = superForm(defaults(data.form.data, zod(isNew ? NewOrganisationReqBody : OrganisationReqBody)), {
      dataType: 'json',
      SPA: true,
...
export const setForm = (data, isNew=false) => setContext(FORM_STATE_KEY, new OrganisationForm(data, isNew));
