...
const HIDDEN_GHOSTERY_PROTOMAPS_LAYER_IDS = new Set([
  'boundaries',
  'boundaries_country',
  'earth_label_islands',
  'roads_tunnels_other',
...
const GHOSTERY_ROAD_BLUE = '#4987E2'
const GHOSTERY_ROAD_BLUE_BRIGHT = '#77AEFF'
const GHOSTERY_DARK = '#141414'
const GHOSTERY_BLACK = '#000000'
const GHOSTERY_VOID = 'rgb(12,12,12)'
const GHOSTERY_BUILDING_FILL = 'rgba(15, 14, 14, 1)'
const GHOSTERY_TEXT_WHITE_SOLID = '#FFFFFF'
...
const createMinorLocalityLayer = (
  ...
  paint: {
    ...(layer.paint ?? {}),
    'text-color': GHOSTERY_TEXT_WHITE_SOLID,
...
const applyGhosteryThemeToProtomaps = (
  ...
  style.layers = style.layers.map(layer => {
    ...
    if (HIDDEN_GHOSTERY_PROTOMAPS_LAYER_IDS.has(layer.id)) {
      return {
        ...layer,
        layout: {
          ...(layer.layout ?? {}),
    ...
    switch (layer.id) {
      case 'background':
        return {
          ...
          paint: {
            ...(layer.paint ?? {}),
            'background-color': GHOSTERY_VOID,
          },
        }
      case 'earth':
        return {
      ...
      case 'buildings':
        return {
          ...
          paint: {
            ...(layer.paint ?? {}),
            'fill-color': GHOSTERY_BUILDING_FILL,
            'fill-opacity': 0.5,
            'fill-outline-color': 'transparent',
          },
        }
        ...
        return {
          ...
          minzoom: layer.id.includes('minor')
            ...
            : layer.id.includes('major')
              ...
              : layer.minzoom,
          paint: {
            'line-color': GHOSTERY_ROAD_BLUE_BRIGHT,
