...
const HIDDEN_NEONMASTER_PROTOMAPS_LAYER_IDS = new Set([
  'boundaries',
  'boundaries_country',
  'earth_label_islands',
  'roads_tunnels_other',
...
const NEONMASTER_ROAD_RED = '#FF355E'
const NEONMASTER_ROAD_RED_BRIGHT = '#FF7A8F'
const NEONMASTER_DARK = '#141414'
const NEONMASTER_BLACK = '#000000'
const NEONMASTER_VOID = 'rgb(12,12,12)'
const NEONMASTER_BUILDING_FILL = 'rgba(15, 14, 14, 1)'
const NEONMASTER_TEXT_WHITE_SOLID = '#FFFFFF'
...
const createGlowLabelLayer = (
  ...
  paint: {
    ...
    'text-halo-color': 'rgb(255, 74, 110)',
...
const createMinorLocalityLayer = (
  ...
  paint: {
    ...(layer.paint ?? {}),
    'text-color': NEONMASTER_TEXT_WHITE_SOLID,
...
const applyNeonmasterThemeToProtomaps = (
  ...
  style.layers = style.layers.map(layer => {
    ...
    if (HIDDEN_NEONMASTER_PROTOMAPS_LAYER_IDS.has(layer.id)) {
      return {
        ...layer,
        layout: {
          ...(layer.layout ?? {}),
    ...
    switch (layer.id) {
      case 'background':
        return {
          ...
          paint: {
            ...(layer.paint ?? {}),
            'background-color': NEONMASTER_VOID,
          },
        }
      case 'earth':
        return {
      ...
      case 'buildings':
        return {
          ...
          paint: {
            ...(layer.paint ?? {}),
            'fill-color': NEONMASTER_BUILDING_FILL,
            'fill-opacity': 0.5,
            'fill-outline-color': 'transparent',
          },
        }
      ...
      case 'roads_bridges_highway':
        return {
          ...
          paint: {
            ...(layer.paint ?? {}),
            'line-color': NEONMASTER_ROAD_RED,
        ...
        return {
          ...
          minzoom: layer.id.includes('minor')
            ...
            : layer.id.includes('major')
              ...
              : layer.minzoom,
          paint: {
            'line-color': NEONMASTER_ROAD_RED_BRIGHT,
      ...
      case 'address_label':
        return {
          ...
          paint: {
            ...
            'text-halo-blur': 1.2,
            'text-halo-color': NEONMASTER_DARK,
            'text-halo-width': 1,
...
export const buildNeonmasterStyle = ({
  noLabels = false,
  locale = 'en',
}: StyleBuildOptions = {}): StyleSpecification => {
  const style = buildNamedProtomapsStyle('dark', { locale })

  applyNeonmasterThemeToProtomaps(style, noLabels, locale)
