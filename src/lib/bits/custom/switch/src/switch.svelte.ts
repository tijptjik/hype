...
export class SwitchRootState {
  ...
  #toggle() {
    const current = this.opts.checked.current

    if (this.opts.states.current === 3) {
      if (current === false) {
        this.opts.checked.current = null
        return
      }
      if (current === null) {
      ...
      this.opts.checked.current = false
      return
    }

    this.opts.checked.current = current !== true
  }
  ...
  readonly sharedProps = $derived.by(() => ({
    ...
    'data-state':
      this.opts.checked.current === null
        ? 'indeterminate'
        : getDataChecked(this.opts.checked.current),
