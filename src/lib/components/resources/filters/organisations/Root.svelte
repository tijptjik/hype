...
let controlMode = $derived(adminCtx.appCtx.state.ui.controlMode.organisation)
let filterControlsElement: HTMLDivElement | undefined = $state()
let sortControlsElement: HTMLDivElement | undefined = $state()
...
const filterKeys: Record<string, (keyof OrganisationViewFilters)[]> = {
  status: ['isPublished', 'isArchived'] as OrganisationStatusFilterKey[],
  image: ['hasImage'] as OrganisationImageFilterKey[],
  authorship: [
    'hasName',
    'hasContextualName',
    'hasDescription',
  ] as OrganisationAuthorshipFilterKey[],
  translation: [
    'isNameTranslated',
...
const getFilterCount = (section: string) => {
  ...
  if (section === 'status') {
    count = filterKeys[section].filter(filterKey => {
      ...
      return (
        organisationFilters[filterKey as OrganisationStatusFilterKey] !== null &&
  ...
  if (section === 'translation') {
    filterKeys[section].forEach(orgKey => {
      const filterValue =
        organisationFilters[orgKey as OrganisationTranslationFilterKey]
    ...
    filterKeys[section].forEach(key => {
      ...
      if (organisationFilters[key as keyof OrganisationViewFilters] !== null) {
        count++
      }
    })
  }
...
async function handleSortOpenChange(isSortOpen: boolean): Promise<void> {
  ...
  if (!filterControlsElement || !sortControlsElement) return

  const hasCollision = areRectsOverlapping(
    filterControlsElement.getBoundingClientRect(),
    sortControlsElement.getBoundingClientRect(),
  )
...
<div
  ...
  transition:slide
>
  {#if controlMode === 'filter' && !isFilterSuppressedByCollision}
    <div
  ...
  {/if}
  <div
    class="flex items-center gap-4 pr-8 text-base-content/60"
    bind:this={sortControlsElement}
  >
    <ResourceSortControl
