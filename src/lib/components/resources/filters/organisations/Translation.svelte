...
// TYPES
import type { OrganisationTranslationFilterKey, Locale } from '$lib/types';
...
const activeLocales = $derived.by(() => {
  ...
  const translationLocales = adminCtx.state.viewFilters.organisation.translationLocales;
...
function toggleLocale(locale: Locale) {
  const current = adminCtx.state.viewFilters.organisation.translationLocales[locale];
  adminCtx.state.viewFilters.organisation.translationLocales[locale] = !current;
}
</script>
...
<div class="flex h-16 flex-row items-center justify-center gap-4">
  ...
  <div class="flex">
    ...
    <div class="flex flex-row gap-3">
      {#each supportedLocales as locale}
        <button
          class="btn btn-xs"
          class:text-white={adminCtx.state.viewFilters.organisation.translationLocales[
            locale
          ]}
          class:btn-secondary={adminCtx.state.viewFilters.organisation
            .translationLocales[locale]}
          class:btn-ghost={!adminCtx.state.viewFilters.organisation.translationLocales[
            locale
          ]}
  ...
  {#each Object.entries(translationFilters) as [filterKey, filterDef], idx (filterKey)}
    {@const currentValue = getResourceFilterState(
      adminCtx,
      'organisation',
      filterKey as OrganisationTranslationFilterKey,
      activeLocales
    )}
    {@const key = filterKey as OrganisationTranslationFilterKey}
    <FilterToggle
      ...
      onToggleFalse={() =>
        toggleResourceFilterState(adminCtx, 'organisation', key, false, activeLocales)}
      onToggleTrue={() =>
        toggleResourceFilterState(adminCtx, 'organisation', key, true, activeLocales)}
      onToggleChange={() => {
        const nextState =
          currentValue === null ? true : currentValue === true ? false : null;
        setResourceFilterState(adminCtx, 'organisation', key, nextState, activeLocales);
      }} />
  {/each}
</div>
