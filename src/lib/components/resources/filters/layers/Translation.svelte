...
<div class="flex h-16 flex-row items-center justify-center gap-4">
  ...
  {#each Object.entries(translationFilters) as [filterKey, filterDef], idx (filterKey)}
    {@const currentValue = getResourceFilterState(
      adminCtx,
      'layer',
      filterKey as LayerTranslationFilterKey,
      activeLocales
    )}
    {@const key = filterKey as LayerTranslationFilterKey}
    <FilterToggle
      ...
      onToggleFalse={() =>
        toggleResourceFilterState(adminCtx, 'layer', key, false, activeLocales)}
      onToggleTrue={() =>
        toggleResourceFilterState(adminCtx, 'layer', key, true, activeLocales)}
      onToggleChange={() => {
        const nextState =
