...
const activeLocales = $derived.by(() => {
  ...
  const translationLocales = adminCtx.state.viewFilters.hub.translationLocales;
...
function toggleLocale(locale: Locale) {
  const current = adminCtx.state.viewFilters.hub.translationLocales[locale];
  adminCtx.state.viewFilters.hub.translationLocales[locale] = !current;
}
</script>
  ...
  <div class="flex">
    ...
    <div class="flex flex-row gap-3">
      {#each supportedLocales as locale}
        <button
          class="btn btn-xs"
          class:text-white={adminCtx.state.viewFilters.hub.translationLocales[locale]}
          class:btn-secondary={adminCtx.state.viewFilters.hub.translationLocales[
            locale
          ]}
          class:btn-ghost={!adminCtx.state.viewFilters.hub.translationLocales[locale]}
