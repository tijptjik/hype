<script lang="ts">
import { m } from '$lib/i18n';
</script>

<section class="leading-7">
  <h3 class="mb-2 font-bold">{m.antsy_patient_moth_taste()}</h3>
  <p>{@html m.home_legal_trout_hurl()}</p>
  <ul class="ml-6 mt-4 list-disc space-y-3">
    <li>
      {@html m.sea_born_mammoth_promise()}
    </li>
    <li>
      {@html m.nimble_tense_guppy_devour()}
    </li>
  </ul>
  <p class="mt-4 italic text-base-content/70">
    {@html m.orange_icy_slug_amaze()}
  </p>
</section>

<section class="leading-7">
  <h3 class="mb-2 font-bold">{m.feature__classification()}</h3>
  <p>
    {m.weak_main_earthworm_thrive()}
  </p>
</section>

<section class="leading-7">
  <h3 class="mb-2 font-bold">{m.feature__specification()}</h3>
  <p>
    {m.less_new_crocodile_dust()}
  </p>
</section>
