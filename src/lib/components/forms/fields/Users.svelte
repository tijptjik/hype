...
// TYPES
import type { UserFieldProps, OrganisationForm, ProjectForm } from '$lib/types'
import type { OrganisationRole } from '$lib/db/zod/schema/organisation.types'
...
const updateUserJoinState = (userId: string, isChecked: boolean) => {
  ...
  userForm.update($form => {
    ...
    const userIndex = userRoles.findIndex((userRole: any) => userRole.userId === userId)
    if (userIndex !== -1) {
      userRoles[userIndex] = {
        ...userRoles[userIndex],
...
const removeUser = async (e: Event, userId: string) => {
  ...
  userForm.update($form => {
    const userRoles = [...($form[fieldRoot] || [])]
    const updatedUsers = userRoles.filter((userRole: any) => userRole.userId !== userId)
    ...
    <div class="grid-span-1 group" in:scale out:scale animate:flip={{ duration: 200 }}>
      ...
      >
        ...
        <div class="card-body flex-auto py-0">
          ...
          >
            <span class="text-xs text-primary"> {userRole.user.attribution} </span>
