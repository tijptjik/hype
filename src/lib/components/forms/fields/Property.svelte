...
const syncComplexValuesToForm = () => {
  formStore.update($formValue => {
    const propertyToUpdateInForm = $formValue[fieldRoot]?.find(p => p.id === propertyId)
...
{#if currentProperty}
  ...
  {#if !collapsed}
    ...
    >
      ...
      {#each supportedLocales as locale, colIndex}
        <div
        ...
        >
          ...
          >
            {#each visibleFields as [ fieldKey, fieldDef ], propIndex}
          ...
          <TranslationBar
            ...
            onTranslate={(e, sourceLocaleFromBar) =>
              translatePropertyAndValues(e, locale, sourceLocaleFromBar)}
            onClear={(e) => clearAllValuesForLocale(e, locale)}
            {isClearAllVisible}
            bind:loadingLocale
