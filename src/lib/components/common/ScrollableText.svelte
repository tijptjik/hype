...
>
	<div class="scroll-wrapper" class:animate={needsScroll} class:needs-scroll={needsScroll}>
		<span class="scroll-primary {textClass}" bind:this={contentSpan}>
			{text}
		</span>
		{#if needsScroll}
			<span class="separator text-neutral-content" style="width: {padding}px;">{separator}</span>
...
<style>
	...
	.scroll-container.overflow-hidden {
		mask-image: linear-gradient(to right, black 0px, black calc(100% - 10px), transparent);
	}
