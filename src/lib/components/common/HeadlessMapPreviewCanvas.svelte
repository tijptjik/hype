...
type HeadlessPreviewKind = 'projects' | 'layers'
...
const getPreviewPadding = (): FitBoundsOptions['padding'] => ({
...
const applyPreviewPayload = (nextPayload: RenderPayload): void => {
...
const loadPreviewPayload = async (): Promise<void> => {
  const currentUrl = new URL(window.location.href)
  const payloadUrl = new URL(
    `/api/mapPreviews/${kind}/${identifier}/render`,
    currentUrl.origin,
  )
  ...
  if (!response.ok) {
    throw new Error(`Failed to load ${kind} preview payload (${response.status})`)
  }

  const nextPayload = (await response.json()) as RenderPayload
  payload = nextPayload
  applyPreviewPayload(nextPayload)
}
...
$effect(() => {
  ...
  void (async () => {
    ...
    await tick()
    fitPreviewViewport(map, nextPayload.features)
...
<div class="headless-map-preview">
...
<div
  ...
  aria-hidden="true"
  class="headless-map-preview__ready"
></div>

<style>
.headless-map-preview {
  width: 100vw;
  height: 100vh;
}

.headless-map-preview__ready {
  position: fixed;
  left: 0;
  top: 0;
  width: 1px;
