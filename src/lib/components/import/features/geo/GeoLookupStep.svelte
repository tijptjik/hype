...
// TYPES
import type { Locale, CSVColumn } from '$lib/types';
...
let addressMetaColumns = $derived.by(() => {
  const columns = importCtx
    .getColumns()
    .filter((col) => col.modelType === 'AddressMeta');
  return columns;
});
...
onMount(() => {
  ...
  // Auto-start geocoding after analysis
  setTimeout(() => {
    if (
      !isGeocoding &&
...
function analyzeGeocodingNeeds() {
  ...
  const rawAddressLocaleToColumnIndex = new Map<Locale, number>();
  const displayAddressLocaleToColumnIndex = new Map<Locale, number>();

  rawAddressColumns.forEach((col) => {
    const columnIndex = headers.findIndex((h) => h === col.header);
    if (columnIndex !== -1 && col.locale) {
      rawAddressLocaleToColumnIndex.set(col.locale as Locale, columnIndex);
    }
  });
  ...
  displayAddressColumns.forEach((col) => {
    const columnIndex = headers.findIndex((h) => h === col.header);
    if (columnIndex !== -1 && col.locale) {
      displayAddressLocaleToColumnIndex.set(col.locale as Locale, columnIndex);
    }
  });
  ...
  function getAddressForLocale(row: string[], locale: Locale): string | null {
  ...
  data.forEach((row, index) => {
    ...
    const hasChineseAddress =
      getAddressForLocale(row, 'zh-hant') !== null ||
      getAddressForLocale(row, 'zh-hans') !== null;
...
async function handleGeocodeForCSV(rawAddress: string, locale: Locale) {
  ...
  try {
    ...
    if (addressComponents.queryAddress) {
      const result = await fetchALSResultFromAddress(addressComponents.queryAddress);
      if (!result) {
      ...
      return toLocalParseResult(addressComponents);
    }
  } catch (error) {
  ...
  for (let i = 0; i < sampledRowIndices.length; i++) {
    ...
    try {
      ...
      // Scenario 2: Chinese address available (no English)
        ...
        const chineseTraditionalData = getAddressForGeocodingWithMeta(row, 'zh-hant');
        const chineseSimplifiedData = getAddressForGeocodingWithMeta(row, 'zh-hans');
      ...
      if (addressForGeocoding) {
        const geocodingResult = await handleGeocodeForCSV(addressForGeocoding, locale);
      ...
      if (i < sampledRowIndices.length - 1) {
...
<div class="flex h-full flex-col">
  ...
  {:else}
    <!-- Progress Bar (when geocoding) -->
    {#if isGeocoding}
      <div class="flex-none p-4 pb-2">
        <div class="mb-2 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <span class="text-sm font-medium">Geocoding Progress</span>
            <span class="text-sm text-base-content/70">
              {currentRow}/{totalRows}
            </span>
          </div>

          <!-- Control button moved to top right -->
          <div class="flex items-center gap-2">
            <button
              class="btn btn-sm {isPaused ? 'btn-success' : 'btn-warning'}"
              onclick={togglePauseResume}
              title={isPaused ? 'Resume geocoding' : 'Pause geocoding'}>
        ...
        <progress
          ...
          value={geocodingProgress.completed + geocodingProgress.errors}
          max={geocodingProgress.total}></progress>
      </div>
    {/if}
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
      {#if isGeocoding && currentRowData}
        ...
        <div class="flex min-h-0 flex-1 gap-4 overflow-hidden p-4">
          ...
          <div class="flex flex-1 flex-col overflow-hidden rounded-lg bg-base-200 p-4">
          ...
          <div class="flex flex-1 flex-col overflow-hidden rounded-lg bg-base-300 p-4">
            <div class="mb-4 flex-none">
              <h5 class="text-sm font-semibold text-base-content/80">
        ...
        <div class="flex-1 p-4">
          <div class="alert alert-info">
            <div>
              ...
              <p>
                Click "Start Geocoding" to begin processing addresses. The system will
                process all rows with address data at 1 request per second.
              </p>
            </div>
          </div>
        </div>
    ...
    <div class="flex-none border-t border-base-content/10 p-4">
      <div class="grid grid-cols-2 gap-6 text-sm">
            ...
            <div class="flex flex-wrap gap-2 font-mono text-xs">
              {#if coordinateColumns.latitude.length > 0 || coordinateColumns.longitude.length > 0}
                {#if coordinateColumns.latitude.length > 0}
                  <span class="badge badge-outline badge-sm">latitude</span>
                {/if}
                {#if coordinateColumns.longitude.length > 0}
                  <span class="badge badge-outline badge-sm">longitude</span>
                {/if}
              {:else}
                <span class="text-base-content/50">None</span>
              {/if}
        ...
        <!-- Progress Stats -->
        <div class="space-y-3">
          <!-- Completed & Errors on same line -->
          <div class="flex gap-3">
            <div
              class="flex flex-1 items-center justify-between rounded-lg bg-success/10 px-3 py-2">
            ...
            <div
              class="flex flex-1 items-center justify-between rounded-lg bg-error/10 px-3 py-2">
              <span class="font-medium text-error">Errors:</span>
              <span class="badge badge-error badge-lg font-mono">
                {geocodingProgress.errors}
              </span>
            </div>
          </div>
          <!-- Matched & No Match on same line -->
          <div class="flex gap-3">
            <div
              class="flex flex-1 items-center justify-between rounded-lg bg-success/10 px-3 py-2">
              <span class="font-medium text-success">Matched:</span>
              <span class="badge badge-success badge-lg font-mono">
                {geocodingProgress.withApiResult}
              </span>
            </div>
            <div
              class="flex flex-1 items-center justify-between rounded-lg bg-warning/10 px-3 py-2">
              <span class="font-medium text-warning">No Match:</span>
              <span class="badge badge-warning badge-lg font-mono">
                {geocodingProgress.withoutApiResult}
              </span>
            </div>
          </div>
          <div
            class="flex items-center justify-between rounded-lg bg-info/10 px-3 py-2">
            <span class="font-medium text-info">Status:</span>
            <span class="badge badge-info badge-lg font-mono">
              {#if isGeocoding}
                {isPaused ? 'Paused' : autoProgress ? 'Auto 1/sec' : 'Manual'}
              {:else}
                Ready
              {/if}
            </span>
      ...
      <!-- Start Button (only when not geocoding) -->
      {#if !isGeocoding}
        <div class="mt-4 flex justify-end">
          {#if addressColumns.length > 0 || coordinateColumns.latitude.length > 0}
            <button class="btn btn-primary" onclick={startGeocoding}>
              Start Geocoding
            </button>
          {:else}
            <button class="btn btn-primary" onclick={skipGeocoding}>
