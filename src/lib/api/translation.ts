...
import {
  PRIVATE_AZURE_TRANSLATION_KEY,
} from '$env/static/private';
import {
  PUBLIC_AZURE_TRANSLATION_REGION,
...
const languageTagToApiLanguageTag = (sourceLang: LanguageTag, targetLang: LanguageTag): { source: string, target: string } => {
  const sourceMaps = {
    'en': 'en',
    'zh-hant': 'yue',
    'zh-hans': 'zh-Hans',
...
export const getTranslation = async (
  ...
  return await fetch(`${ENDPOINT}/translate?api-version=3.0&from=${source}&to=${target}`, {
    method: 'POST',
    headers: {
