...
const { form, errors, message, constraints, enhance, validateForm } = superForm(
  defaults(data.form, zod(LayerSchema)),
  {
    dataType: 'json',
    SPA: true,
    validators: zod(LayerSchema),
    onSubmit: async ({ action, formData, formElement, controller, submitter, cancel }) => {
      const response = await fetch(`/api/layers/${$form.id}`, {
        method: 'PUT',
        headers: {
...
<main class="flex flex-col">
  ...
  <div>
    <h1 class="text-lg">Edit Layer</h1>
        ...
        <div class="items-star flex w-1/3 flex-col">
          ...
          <div>
            ...
            <textarea
              ...
              class:textarea-error={errors.metadata}
              aria-invalid={errors.metadata ? 'true' : undefined}></textarea>
            {#if errors.metadata}<span class="text-sm text-error">{errors.metadata}</span>{/if}
          </div>
        </div>
      {/snippet}
