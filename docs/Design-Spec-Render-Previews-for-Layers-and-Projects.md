...
5. Add a dedicated headless preview route.
   Create something like `/headless/map-layer-preview/[layerId]` that renders only the deterministic preview surface, no app chrome, and emits the same ready marker used by the current `mapPreviews/styles` renderer flow.
...
6. Extend the queue job builder for layers.
   ...
   - writes object keys as `mapPreviews/layers/{layerId}/{hash}.png`
   - fits under the `api/mapPreviews` namespace, where `mapPreviews/styles` already handles built-in style previews today
